...
# Use a simple FileHandler without rotation to avoid PermissionError
handler = logging.FileHandler('oauth_debug.log')
...
def auth_callback():
    ...
    try:
        ...
        logger.debug(f"Access token: {credentials.token[:10]}...")
...
# Import the dashboard setup function
from dashboard_setup import create_dashboard

# Create the dashboard app
flask_app = app  # Reference to the Flask app

dashboard_app = create_dashboard(flask_app)

dashboard_app.css.config.serve_locally = True
dashboard_app.scripts.config.serve_locally = True
